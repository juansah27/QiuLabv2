...
const calculateModalStats = (dataArray, type) => {
...
// Popup Modal Component - Redesigned
const DetailModal = ({ isOpen, onClose, title, data, fullData = null, loading = false, isMockData = false }) => {
  ...
  const dataConfig = useMemo(() => {
    ...
    // Check if it's TTS LABEL data - Specific handling for layout requirements
    if (firstItem.Client && firstItem.ORDNUM && firstItem['Order Status'] && firstItem['Payment Date']) {
      return {
        ...
        summaryStats: calculateModalStats(data, 'ttsLabel'),
    ...
    if (firstItem.MARKETPLACE && firstItem.Brand && firstItem.SystemRefId) {
      return {
        ...
        summaryStats: calculateModalStats(data, 'main')
      };
    }

    // Check if it's SKU Telat Masuk 2 data
    if (firstItem.Brand && firstItem['Order NUmber'] && firstItem['Order Status'] && firstItem['Quantity SOL'] !== undefined && firstItem['Quantity XML'] !== undefined) {
      return {
        ...
        summaryStats: calculateModalStats(data, 'lateSku2')
      };
    }
    ...
    if (firstItem.ORDNUM && firstItem.PRTNUM && firstItem.ORDLIN !== undefined && firstItem.ORDSLN !== undefined) {
      return {
        ...
        summaryStats: calculateModalStats(data, 'lateSku')
      };
    }
    ...
    if (firstItem.Client && firstItem.ORDNUM && firstItem.Issue) {
      return {
        ...
        summaryStats: calculateModalStats(data, 'invalidSku')
      };
    }
    ...
    return {
      headers,
      summaryStats: calculateModalStats(data, 'default')
    };
  }, [data]);

  const { headers, summaryStats, topFiveStats } = dataConfig;

  // Helper to get max value for progress bars
  const maxTopFiveValue = useMemo(() => {
    if (!topFiveStats || topFiveStats.length === 0) return 0;
    return Math.max(...topFiveStats.map(s => s.value));
  }, [topFiveStats]);
  ...
  const copyAllData = async () => {
    ...
    // Use requestAnimationFrame to allow UI update to 'copying' state
    // but keep it linked to the user gesture event loop as much as possible
    ...
    try {
      ...
      try {
        ...
        // Ensure it's part of the layout but hidden
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col relative animate-fade-in ring-1 ring-gray-200 dark:ring-gray-700"
        onClick={(e) => e.stopPropagation()}
      >
        {/* 1. HEADER BAR */}
        <div className="flex items-center justify-between px-7 py-5 border-b border-gray-100 dark:border-gray-700/50 bg-white dark:bg-gray-800 sticky top-0 z-20">
          <div className="flex items-center space-x-3">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">{title}</h2>
            {isMockData && (
              <div className="px-2.5 py-0.5 bg-orange-50 text-orange-700 border border-orange-200 dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-700/50 rounded-full text-[10px] font-bold uppercase tracking-wider">
                Mock Data
              </div>
            )}
          </div>
          <button
            onClick={onClose}
            className="p-2 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 bg-gray-50 hover:bg-gray-100 dark:bg-gray-700/50 dark:hover:bg-gray-700 rounded-lg transition-all duration-200"
            title="Close (ESC)"
          >
        ...
        <div className="flex-1 overflow-y-auto px-7 py-6 custom-scrollbar">
          {loading ? (
            ...
            <div className="space-y-8">
              ...
              {/* 2. SUMMARY ROW - Info Cards */}
              <div className={`grid gap-6 w-full ${summaryStats.length > 2 ? 'grid-cols-2 md:grid-cols-4' : 'grid-cols-2'}`}>
                {summaryStats.map((stat, index) => (
                  <div key={index} className={`bg-${stat.color}-50/50 dark:bg-${stat.color}-900/10 p-5 rounded-xl border border-${stat.color}-100 dark:border-${stat.color}-800/30 flex flex-col items-center justify-center text-center`}>
                    <div className={`text-xs font-bold uppercase tracking-widest text-${stat.color}-600/70 dark:text-${stat.color}-400/70 mb-2`}>{stat.label}</div>
                    <div className={`text-3xl font-black text-${stat.color}-700 dark:text-${stat.color}-300 tracking-tight`}>{typeof stat.value === 'number' ? stat.value.toLocaleString() : stat.value}</div>
                  </div>
                ))}
              </div>

              {/* 3. TOP 5 CLIENTS SECTION */}
              {topFiveStats && topFiveStats.length > 0 && (
                <div className="space-y-4">
                  <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Top 5 Clients</h3>
                  <div className="grid grid-cols-5 gap-4">
                    {topFiveStats.map((stat, index) => {
                      const percentage = maxTopFiveValue > 0 ? (stat.value / maxTopFiveValue) * 100 : 0;
                      const isFirst = index === 0;

                      return (
                        <div key={index} className={`relative bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border ${isFirst ? 'border-purple-200 dark:border-purple-800/50 ring-1 ring-purple-100 dark:ring-purple-900/20' : 'border-gray-100 dark:border-gray-700/50'} flex flex-col justify-between h-full hover:shadow-sm transition-shadow`}>
                          {isFirst && <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-purple-400 to-pink-400 rounded-t-xl opacity-80"></div>}

                          <div className="flex flex-col mb-3">
                            <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 truncate mb-1" title={stat.label}>
                              {index + 1}. {stat.label}
                            </div>
                            <div className={`text-xl font-bold ${isFirst ? 'text-purple-700 dark:text-purple-300' : 'text-gray-900 dark:text-white'}`}>
                          ...
                          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                            <div
                              className={`h-full rounded-full ${isFirst ? 'bg-gradient-to-r from-purple-500 to-pink-500' : 'bg-gray-400 dark:bg-gray-500'}`}
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest leading-none mb-1">Recent Orders</h3>

                  <button
                    onClick={copyAllData}
                    disabled={copySuccess === 'copying'}
                    className={`group flex items-center space-x-2 px-3 py-1.5 text-xs font-semibold rounded-lg transition-all duration-200 ${copySuccess === 'all'
                      ? 'bg-green-50 text-green-700 border border-green-200'
                      : copySuccess === 'copying'
                        ? 'bg-gray-50 text-gray-500'
                        : 'bg-white text-gray-600 border border-gray-200 hover:border-blue-300 hover:text-blue-600 hover:shadow-sm dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:hover:border-blue-500/50'
                      }`}
                  >
                    {copySuccess === 'copying' ? (
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden bg-white dark:bg-gray-800 shadow-sm relative">
                  {/* Table Scroll Area */}
                  <div className="overflow-x-auto max-h-[400px] custom-scrollbar">
                    <table className="min-w-full divide-y divide-gray-100 dark:divide-gray-700/50">
                      <thead className="bg-gray-50/50 dark:bg-gray-700/30 sticky top-0 z-10 backdrop-blur-sm">
                        <tr>
                          {headers.map((header) => (
                            <th
                              key={header}
                              className={`px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider cursor-pointer group select-none transition-colors ${selectedColumn === header
                            ...
                            >
                              <div className="flex items-center space-x-1">
                                ...
                                {copySuccess === header && (
                                  <span className="text-green-500 animate-fade-in"><CheckCircleIcon className="w-3.5 h-3.5" /></span>
                                )}
                              </div>
                            </th>
                          ))}
                      ...
                      <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-50 dark:divide-gray-700/50">
                        {data.slice(0, 1000).map((item, index) => (
                          <tr key={index} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors duration-150">
                            {headers.map((header) => (
                              <td key={header} className="px-6 py-3 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                                {header === 'OrderDate' || header === 'Payment Date' ? (
                                  <span className="font-mono text-xs text-gray-500 dark:text-gray-400">
                                    {new Date(item[header]).toLocaleString(undefined, {
                                      year: 'numeric', month: 'short', day: 'numeric',
                                      hour: '2-digit', minute: '2-digit'
                                    })}
                                  </span>
                                ) : header === 'ORDER STATUS' || header === 'Order Status' || header === 'Status_Interfaced' ||
                                  header === 'Status_Durasi' || header === 'Interface' || header === 'LineStatus' || header === 'Issue' ? (
                                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium border border-opacity-20 ${item[header] === 'Yes' || item[header] === 'Match'
                                    ? 'bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-800'
                                    : item[header] === 'No' || item[header] === 'Mismatch'
                                      ? 'bg-red-50 text-red-700 border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800'
                                      : 'bg-gray-50 text-gray-600 border-gray-200 dark:bg-gray-700/50 dark:text-gray-400 dark:border-gray-600'
                                    }`}>
                                    <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${item[header] === 'Yes' || item[header] === 'Match' ? 'bg-green-500' :
                                      item[header] === 'No' || item[header] === 'Mismatch' ? 'bg-red-500' :
                                        'bg-gray-400'
                                      }`}></span>
                                    {item[header]}
                                  </span>
                                ) : (
                                  <span className="font-medium text-gray-700 dark:text-gray-200">{item[header]}</span>
                                )}
                              </td>
                            ))}
                          </tr>
                  ...
                  {data.length > 1000 && (
                    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-white/90 dark:bg-gray-800/90 backdrop-blur px-4 py-2 rounded-full shadow-lg border border-yellow-200 dark:border-yellow-700/50 flex items-center space-x-2 animate-bounce-subtle">
                      <ExclamationTriangleIcon className="w-4 h-4 text-yellow-500" />
                      <span className="text-xs font-medium text-yellow-700 dark:text-yellow-400">
                        Showing first 1,000 of {data.length.toLocaleString()} records
                      </span>
                    </div>
                  )}
                </div>
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">No data available</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 max-w-xs">There are no records matching the selected criteria.</p>
            </div>
          )}
        </div>

        {/* 5. FOOTER BAR */}
        <div className="flex items-center justify-between px-7 py-5 border-t border-gray-200 dark:border-gray-700/50 bg-gray-50/50 dark:bg-gray-800/50 sticky bottom-0 z-20 backdrop-blur-sm">
          <div className="text-sm font-medium text-gray-500 dark:text-gray-400">
            Showing <span className="text-gray-900 dark:text-white font-bold">{data ? data.length.toLocaleString() : 0}</span> records
          </div>
          <button
            onClick={onClose}
            className="px-6 py-2.5 text-sm font-semibold text-white bg-gray-900 hover:bg-black dark:bg-white dark:text-gray-900 dark:hover:bg-gray-100 rounded-xl shadow-sm transition-all duration-200 hover:shadow transform hover:-translate-y-0.5 active:translate-y-0"
          >
            Close
          </button>
        </div>
