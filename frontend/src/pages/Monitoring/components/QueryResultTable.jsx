...
const QueryResultTable = ({ 
  ...
  const allColumns = useMemo(() => {
    ...
    availableKeys.forEach(key => {
      // Mapping key asli ke versi yang sudah diformat (lower case dan tanpa underscore)
      keyMapping[key.toLowerCase().replace(/_/g, '')] = key;
    });
  ...
  const renderCellValue = (value, row, column, rowIndex) => {
    if (column.editable) {
      ...
      return (
        <div 
          className={`group cursor-pointer px-2 py-1 ${isDarkMode ? 'bg-blue-900/30' : 'bg-blue-50'} rounded ${
            isEdited ? `font-medium ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}` : ''
  ...
  return (
    <div className="relative">
      ...
      <div className={`w-full ${THEME_COLORS.card.light} ${THEME_COLORS.card.dark} rounded-lg overflow-hidden shadow-sm ${THEME_TRANSITIONS.default}`}>
          ...
          <div className="flex items-center gap-2 mb-3 md:mb-0">
            ...
            {onRefresh && (
              <button
                ...
                disabled={loading}
                className={`px-2 py-1 text-xs rounded-md border ${THEME_COLORS.inputBorder.light} ${THEME_COLORS.inputBorder.dark} ${
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="mt-1 relative">
              <input
                ...
                className={`w-full px-2 py-1 pl-7 text-xs border ${THEME_COLORS.inputBorder.light} ${THEME_COLORS.inputBorder.dark} rounded ${
                ...
                <tr>
                  {columns.map((column, colIndex) => (
                    <th 
                      key={column.id}
                      scope="col"
                      className={`${isCompactView ? 'px-2 py-1' : 'px-3 py-2'} text-xs font-medium text-left ${
                        isDarkMode ? 'text-gray-300' : 'text-gray-500'
                      } select-none cursor-pointer ${
                        selectedColumns.includes(column.id) ? (isDarkMode ? 'bg-blue-900/30' : 'bg-blue-100') : ''
                      } ${THEME_TRANSITIONS.default}`}
                  ...
                  >
                    {columns.map((column, colIndex) => (
                      <td 
                        ...
                        className={`${isCompactView ? 'px-2 py-1' : 'px-3 py-2'} whitespace-nowrap ${
        ...
        {/* Footer */}
        <div className={`flex justify-between items-center px-4 py-2 border-t ${THEME_COLORS.border.light} ${THEME_COLORS.border.dark} ${
