...
def load_orders(filepath):
    ...
    for line in lines:
        ...
        if len(parts) == 2:
            ...
            if not (token_upper.startswith('TTS') or token_upper.startswith('579')):
                continue
            clean_order = full_order.strip()
            if clean_order.upper().startswith('TTS'):
...
def run_exe_parallel(brand_folder):
    exe_path = Path(brand_folder) / "tiktok.api.exe"
