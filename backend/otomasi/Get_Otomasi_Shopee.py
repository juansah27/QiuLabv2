...
from path_helper import get_shopee_path

# Path utama folder Shopee - Cross-platform support
BASE_PATH = get_shopee_path()
...
def load_orders(filepath):
    ...
    for line in lines:
        ...
        if len(parts) == 2:
            ...
            if not token_upper.startswith('SHOPEE'):
                continue
            clean_order = full_order.strip()[len('SHOPEE'):].strip()
...
def get_best_matching_folder(base_path, brand_name):
    ...
    if not best_match:
        print(f"[!] WARNING: Tidak ada folder cocok untuk brand: {brand_name} (dibersihkan: {cleaned_brand})")
    return best_match


def run_exe_parallel(brand_folder):
    exe_path = Path(brand_folder) / "ShopeeOrderLogistic.exe"
    if not exe_path.exists():
        print(f"[X] ERROR: ShopeeOrderLogistic.exe tidak ditemukan di: {exe_path}")
        return

    print(f"[*] INFO: Menjalankan .exe di: {brand_folder}")
    print(f"[*] Command: ShopeeOrderLogistic.exe")
    ...
    cmd = f'pushd "{brand_folder}" && ShopeeOrderLogistic.exe && popd'
...
def main():
    ...
    for brand, order_numbers in brand_to_orders.items():
        ...
        if not brand_folder.exists():
            print(f"[X] SKIP: Folder tidak ditemukan: {brand_folder}")
            continue
        ...
        print(f"[OK] SUCCESS: Order untuk brand '{brand}'")
