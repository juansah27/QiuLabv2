...
from path_helper import get_lazada_path

BASE_PATH = get_lazada_path()
    ...
    for line in lines:
        ...
        if len(parts) == 2:
            ...
            if not (token_upper.startswith('LZ') or token_upper.startswith('264') or token_upper.startswith('273')):
                continue
            without_lz = full_order.strip()
            if without_lz.upper().startswith('LZ'):
                without_lz = without_lz[2:]
            clean_order = without_lz.split('-')[0].strip()
...
def run_exe_parallel(brand_folder):
    exe_path = Path(brand_folder) / "LazadaMarketplace.exe"
    if not exe_path.exists():
        print(f"[X] ERROR: LazadaMarketplace.exe tidak ditemukan di: {exe_path}")
        return
    ...
    if system == "Linux":
        ...
        if wine_check.returncode == 0:
            exe_command = "wine LazadaMarketplace.exe"
        ...
        exe_command = "LazadaMarketplace.exe"
